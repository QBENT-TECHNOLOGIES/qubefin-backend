...
#region --- HANDLER ---
internal sealed class GetUserLoginInfoQueryHandler(
    QubeFinDataContext context
) : IRequestHandler<
    GetUserLoginInfoQuery,
    Result<UserLoginInfoResponse>>
{
    public async Task<Result<UserLoginInfoResponse>> Handle(
        GetUserLoginInfoQuery request,
        CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            return new RecordNotFoundError(
                $"User not found for the given Id: {request.Id}");
        }

        var accessOrganizationUnits =
            user.Employee?.OrganizationUnit?.Latitude != null
                ...
                : await GetUserOrganizationUnits(
                    user.Employee?.OrganizationUnitId ?? Guid.Empty,
                    cancellationToken);
    ...
    private async Task<List<UserAccessOrganizationUnit>> GetUserOrganizationUnits(
        Guid orgUnitId,
        CancellationToken cancellationToken)
    {
